...
|-----------|---------|----------------|---------------|
| GDPR (EU) | Yes | Article 9: health data requires explicit consent + data minimisation | Consent ✅ · UUID pseudonymisation ✅ · log filtering ✅ · table segregation ⚠️ P2 |
...
### P1 — Implemented 21 April 2026

#### 1. Pseudonymous UUID (`public_id`)

**Migration:** `db/migrate/20260421100104_add_public_id_to_users.rb`
...
Every user row now carries a `public_id` — a random UUID (e.g. `a3f7c219-84b1-4e02-9a17-...`) generated by PostgreSQL's `gen_random_uuid()` function. It is:

- **Not guessable** — cryptographically random v4 UUID, not sequential
- **Not the database primary key** — `id` stays internal; `public_id` is for external reference only
- **Stable** — generated once at row creation, never changes
- **Automatically assigned** — the DB default means no application code is required; every new user gets one without any Rails model change

**What existing users got:** PostgreSQL generated a unique UUID for every existing row at migration time. No backfill job was needed.

**How to use it:**

```ruby
# In admin CSV exports — reference public_id, not id or email
user.public_id  # => "a3f7c219-84b1-4e02-9a17-c1b3e2f90d44"

# In error tracking (Sentry) — tag with public_id, never email
Sentry.set_user(id: current_user.public_id)

# In Trello card mailer — use public_id as the user reference
"Submitted by user #{user.public_id}"
```

**What it does NOT do:**

- It does not change any existing routes — all app routes still use `id`
- It does not pseudonymise data already stored — that requires the P2 health profile segregation
- It does not replace the need for the `user_health_profiles` table split

---

#### 2. Health Data Filter Parameters
...
Rails replaces filtered parameter values with `[FILTERED]` in the log. The parameter name still appears so you can debug routing — only the value is hidden.

```log
# Before — health data visible in plain text:
Parameters: {"symptom_log"=>{"mood"=>"4", "weight"=>"62.5", "notes"=>"feeling tired"}}

# After — values redacted:
Parameters: {"symptom_log"=>{"mood"=>"[FILTERED]", "weight"=>"[FILTERED]", "notes"=>"[FILTERED]"}}
```

This applies to:

- Rails development log (`log/development.log`)
- Production log on Render
- Any log aggregation service connected in future (Datadog, Betterstack, etc.)
- Sentry breadcrumbs that include request parameters

**`:notes` is filtered globally.** This is intentional — `notes` fields appear on `symptom_logs` and `calendar_events`, both of which can contain sensitive free text written by the user. The cost of filtering it everywhere outweighs the debugging inconvenience.
...
| P3 | Update Trello card mailer to use `public_id`, strip health data | 30 min | ⬜ Open |
| P4 | App Store / Play Store privacy disclosures | 1 hr | ⬜ Before store submission |
