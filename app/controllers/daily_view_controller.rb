class DailyViewController < ApplicationController
  def show
    ...
    @all_day_events = @events.select { |e| e.start_time.nil? || (e.start_time.hour == 0 && e.start_time.min == 0 && e.end_time && e.end_time.hour == 23 && e.end_time.min == 59) }
    @timed_events = @events - @all_day_events
