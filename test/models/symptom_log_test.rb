...
class SymptomLogTest < ActiveSupport::TestCase
  ...
  test "is valid with a date and user" do
    log = SymptomLog.new(user: @alice, date: Time.zone.today)
    assert log.valid?
  end

  test "is invalid when same user logs twice on the same date" do
    SymptomLog.create!(user: @alice, date: Time.zone.today)
    duplicate = SymptomLog.new(user: @alice, date: Time.zone.today)
  ...
  test "allows two different users to log on the same date" do
    SymptomLog.create!(user: @alice, date: Time.zone.today)
    bob_log = SymptomLog.new(user: @bob, date: Time.zone.today)
